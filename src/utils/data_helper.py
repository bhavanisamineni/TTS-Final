...
def convert_data_to_ids(tokenizer, target, text, label, config):
    ...
    # concat_sent = [f"{ele[0]} </s> {ele[1]}" for ele in concat_sent]
    # print(concat_sent[:2])
    ...
    # print(encoded_dict["input_ids"][:2])
    # print(encoded_dict['gt_label'])
    # raise Exception
...
def data_helper_bert(x_train_all,x_val_all,x_test_all,x_test_kg_all,model_select,config):
    ...
    if model_select == 'Bertweet':
        tokenizer = BertweetTokenizer.from_pretrained("vinai/bertweet-base", normalization=True, clean_up_tokenization_spaces=True)
